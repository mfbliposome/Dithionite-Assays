...
def analyze_fluorescence_decay_triton(data, filename, base_results_dir, time_range=60, os_system='Mac'):
    '''
    ...
    if os_system == 'Mac':
        ...
        results_dir = os.path.join(base_results_dir, f"{filename}_{timestamp}")
...
def analyze_fluorescence_decay_no_triton_fixed_kout(
    data, 
    filename, 
    k_deg_out_fixed,   
    base_results_dir, 
    ...
    """
    Fit fluorescence decay curves (no Triton) with k_deg_out fixed.
    ...
    k_deg_out_fixed : float
        Known degradation rate constant (fixed, not fitted).

    p0 : list
        Initial guess for [C1, C2, k_deg_in, k_perm].
    ...
    # Updated function with fixed k_deg_out
    def F_total_fixed(t, C1, C2, k_deg_in, k_perm):
        term_in = np.exp(-k_deg_in * (t + np.exp(-k_perm * t) / k_perm))
        term_out = np.exp(-k_deg_out_fixed * t)
        return C1 * term_out + C2 * term_in
    ...
    for i, col in enumerate(columns):
        ...
        try:
            ...
            F_fit = F_total_fixed(t, *popt)

            # Compute metrics
            r2 = r2_score(F, F_fit)
            rmse = np.sqrt(mean_squared_error(F, F_fit))

            results.append((col, popt[0], popt[1], k_deg_out_fixed, popt[2], popt[3], r2, rmse))
            ...
            print(f"Fit failed for {col}")
            results.append((col, *[np.nan]*2, k_deg_out_fixed, *[np.nan]*2, np.nan, np.nan))

    plt.xlabel('Time')
    plt.ylabel('Fluorescence')
    plt.title('Fluorescence Decay Fit (No Triton, k_out fixed)')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    fit_plot_filename = f"{filename}_fit_plot_noTriton_fixedkout.png"
    plot_path = os.path.join(results_dir, fit_plot_filename)
    plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    plt.close()
    ...
    for i, (col, *params) in enumerate(results):
        if np.isnan(params[0]):
            continue
        F = data[col].values
        # rebuild params: [C1, C2, k_in, k_perm]
        F_fit = F_total_fixed(t, params[0], params[1], params[3], params[4])
        residuals = F - F_fit
        plt.scatter(t, residuals, s=10, label=f'{col} residuals', alpha=0.7)
    ...
    plt.xlabel("Time")
    plt.ylabel("Residuals")
    plt.title("Residual Plot (No Triton, k_out fixed)")
    plt.legend()
    plt.grid(True)
    residual_plot_filename = f"{filename}_residual_plot_noTriton_fixedkout.png"
    plot_path = os.path.join(results_dir, residual_plot_filename)
    plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    plt.close()
    ...
    csv_filename = f"{filename}_fit_params_noTriton_fixedkout.csv"
    csv_path = os.path.join(results_dir, csv_filename)
    param_df.to_csv(csv_path, index=False)
...
def analyze_fluorescence_decay_no_triton_numerical(data, filename, ub=[np.inf, np.inf, 10, 10, 10], os_system='Mac'):
    ...
    for i, fluorescence in enumerate(fluorescence_trials):
        ...
        try:
            ...
            params_list.append((f"{columns[i]}", *[np.nan]*5, np.nan, np.nan))
    ...
    for i, (fluorescence, F_fit) in enumerate(zip(fluorescence_trials, fit_curves)):
        plt.scatter(time, fluorescence, color=colors[i], label=f'{columns[i]} (data)', alpha=0.3)
        plt.plot(time, F_fit, '-', color=colors[i], label=f'{columns[i]} (fit)')
    plt.xlabel("Time")
    plt.ylabel("Fluorescence")
    ...
    for i, residuals in enumerate(residuals_list):
        plt.scatter(time, residuals, s=10, color=colors[i], label=f'{columns[i]}', alpha=0.7)
...
def analyze_fluorescence_decay_no_triton_numerical_fixed_kout(data, filename, k_deg_out_fixed, base_results_dir, ub = [np.inf, np.inf, 10, 10], os_system='Mac'):
    ...
    # Define ODE model
    def fluorescence_model(t, F_out0, F_in0,  k_deg_in, k_perm):
        def odes(t, F):
            F_out, F_in = F
            dF_out_dt = -k_deg_out_fixed * F_out
            dF_in_dt = -k_deg_in * F_in * (1 - np.exp(-k_perm * t))
            return [dF_out_dt, dF_in_dt]
    ...
    def fit_function(t, F_out0, F_in0, k_deg_in, k_perm):
        return fluorescence_model(t, F_out0, F_in0, k_deg_in, k_perm)
    ...
    # for i, fluorescence in enumerate(columns):
        ...
        try:
            ...
            params_list.append((columns[i], popt[0], popt[1], k_deg_out_fixed,
                                popt[2], popt[3], r2, rmse))
            fit_curves.append(F_fit)
            residuals_list.append(residuals)

        except RuntimeError:
            print(f"Fit failed for Trial {i+1}")
            params_list.append((columns[i], *[np.nan]*5, np.nan, np.nan))
            fit_curves.append(np.full_like(time, np.nan))
            residuals_list.append(np.full_like(time, np.nan))
